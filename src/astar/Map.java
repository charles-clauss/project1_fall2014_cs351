...
class Map {
	 ...
	 */
	public Map(int x, int y, int pathCost) {
		this.x = x;
		this.y = y;
	 ...
	 *            monotonic.
	 */
	public Map(int x, int y, int pathCost, Map origin) {
		this.x = x;
		this.y = y;
		this.pathCost = pathCost;
		this.fullCost = AStar.cost(x, y, pathCost);
	...
	public boolean equals(Object o) {
		if (o instanceof Map) {
			if (((Map) o).getX() == this.x && ((Map) o).getY() == this.y) {
				return true;
			}
		}
		return false;
	 ...
	 * @see Astar
	 */
	class SortMapData implements Comparator<Map> {
